...
use orvpass_core::import_export::parsers::{
    apple::parse_apple_csv, bitwarden::parse_bitwarden_json, chrome::parse_chrome_csv,
    keepass::parse_keepass_csv, lastpass::parse_lastpass_csv, onepassword::parse_1password_csv,
};
use std::fs;
...
pub fn execute(file: &str) {
    if let Ok(content) = fs::read_to_string(file) {
        ...
        if count > 0 {
            ...
            println!(
                "✨ Successfully imported {} items from '{}' into encrypted vault.",
                count, file
            );
        } else {
            println!(
                "ℹ️  File '{}' read successfully (0 valid credentials found).",
                file
            );
        }
    } else {
