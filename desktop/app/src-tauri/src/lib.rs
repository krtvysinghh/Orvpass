...
fn initialize_vault(state: State<AppState>, password: &str) -> Result<String, String> {
    let path = dirs::home_dir().unwrap().join(".orvpass").join("tauri_vault.enc");
...
#[tauri::command]
fn add_item(state: State<AppState>, title: String, username: String, pass: String) -> Result<(), String> {
    let mut guard = state.vault_data.lock().unwrap();
    if let Some((v, key)) = &mut *guard {
        let login = LoginData { username: Some(username), password: Some(pass), urls: vec![] };
...
pub fn run() {
    tauri::Builder::default()
        .manage(AppState { vault_data: Mutex::new(None) })
        .setup(|app| {
