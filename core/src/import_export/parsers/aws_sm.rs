...
pub fn parse_aws_secrets_json(json_str: &str) -> Vec<VaultItem> {
    let mut items = Vec::new();
    if let Ok(root) = serde_json::from_str::<Value>(json_str) {
        if let Some(list) = root.get("SecretList").and_then(|v| v.as_array()) {
            for s in list {
                let name = s.get("Name").and_then(|v| v.as_str()).unwrap_or("AWS Secret");
